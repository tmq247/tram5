...
async def get_thumb(videoid: str) -> str:
    ...
    try:
        ...
        data = result_items[0]
        title = re.sub(r"\W+", " ", data.get("title",
    ...
    try:
        title_font = ImageFont.truetype(
            "HasiiMusic/assets/thumb/font2.ttf", 32)
        regular_font = ImageFont.truetype(
            "HasiiMusic/assets/thumb/font.ttf", 18)
    except OSError:
    ...
    draw.text((TITLE_X, TITLE_Y), trim_to_width(title, title_font,
              MAX_TITLE_WIDTH), fill="black", font=title_font)
    draw.text((META_X, META_Y),
              f"YouTube | {views}", fill="black", font=regular_font)
    ...
    if os.path.isfile(icons_path):
        ...
        r, g, b, a = ic.split()
        black_ic = Image.merge("RGBA", (r.point(
            lambda *_: 0), g.point(lambda *_: 0), b.point(lambda *_: 0), a))
